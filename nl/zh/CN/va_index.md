...
1.  [在集群中设置 Helm](/docs/containers/cs_integrations.html#helm)。如果使用 RBAC 策略来授予 Helm Tiller 访问权，请确保 Tiller 角色有权访问所有名称空间。授予 Tiller 角色访问权，以确保容器扫描程序可以监视所有名称空间中的容器。
...
4.  编辑 `config.yaml` 文件。
    ...
    <td>将 <code>ClusterID</code> 替换为要在其中安装容器扫描程序的 Kubernetes 集群。要列出集群标识，请运行 <code>ibmcloud ks clusters</code>。<br> **提示**：请使用集群的标识，而不是集群的名称。
    </td>
    </tr>
    <tr>
...
4.  展开**漏洞顾问程序**并单击**扫描的存储库**。
5.  要查看标记为 `latest` 的映像的报告，请单击该存储库的行。报告显示问题总数以及它们是有漏洞的包还是配置问题。如果存储库中不存在 `latest` 标记，那么将使用最新的映像。
6.  要查看所选映像的每个有漏洞包的信息，请在**发现的有漏洞包**表中，单击**漏洞**列中的链接以打开报告。
    1.  要查看更多信息，请展开摘要。
    2.  如果提供了操作系统分发者的声明，请单击**官方声明**列中的链接。
7.  要查看每个配置问题的信息，请在**发现的配置问题**表中，单击问题的行。
8.  为报告中显示的每个问题完成纠正行动并重建映像。
...
4.  决定如何更新容器以便您能够解决问题。
    ...
    **重要信息**：要解决容器映像问题，必须删除旧实例并重新部署，这意味着会丢失现有容器中的所有数据。请确保对容器体系结构有充分的了解，以选择相应的容器重新部署方法。
    ...
    **示例**
    ...
    -   在微服务体系结构中，可在解决安全或配置问题时，将流量路由到其他容器实例，然后在红/黑部署中推送新映像。
